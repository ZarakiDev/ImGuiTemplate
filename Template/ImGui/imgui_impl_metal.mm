...
- (id<MTLRenderPipelineState>)_renderPipelineStateForFramebufferDescriptor:(FramebufferDescriptor *)descriptor device:(id<MTLDevice>)device
{
    ...
    "                             constant bool& sRGB [[buffer(1)]]) {\n"
    "    constexpr sampler linearSampler(coord::normalized, min_filter::linear, mag_filter::linear, mip_filter::linear);\n"
    "    half4 texColor = texture.sample(linearSampler, in.texCoords);\n"
    "    if (sRGB) {\n"
    "        texColor = half4(pow(texColor.rgb, 1.0 / 2.2), texColor.a);\n"
    "    }\n"
            ...
            else
            {
                ...
                if (clip_rect.x < fb_width && clip_rect.y < fb_height && clip_rect.z >= 0.0f && clip_rect.w >= 0.0f)
                {
                    ...
                    if (ImTextureID tex_id = pcmd->GetTexID())
                    {
                        ...
                        srgb = texture.pixelFormat == MTLPixelFormatASTC_6x6_sRGB;
                    }

                    [commandEncoder setFragmentBytes:&srgb length:sizeof(bool) atIndex:1];
