...
int32_t CGPInstructionDecoder::GetBit(uint32_t insn, int pos)
{
    return (insn >> pos) & 1;
}

int32_t CGPInstructionDecoder::GetBits(uint32_t insn, int pos, int length) 
{
    return (insn >> pos) & ((1 << length) - 1);
}

bool CGPInstructionDecoder::IsADR(uint32_t insn)
{
    return (insn & 0x9F000000) == 0x10000000;
}

bool CGPInstructionDecoder::IsADRP(uint32_t insn)
{
    return (insn & 0x9F000000) == 0x90000000;
}

bool CGPInstructionDecoder::IsLDR(uint32_t insn)
{
    return GetBit(insn, 22) == 1;
}

bool CGPInstructionDecoder::IsSTR(uint32_t insn)
{
    return (insn & 0xFFC00000) == 0x39000000;
}

bool CGPInstructionDecoder::IsBL(uint32_t insn)
{
    // Branch-with-Link if top-6 bits == 100101
    return (insn & 0xFC000000u) == 0x94000000u;
}

bool CGPInstructionDecoder::IsLDRSTU(uint32_t insn)
{
    return (insn & 0x0A000000) == 0x08000000;
}

bool CGPInstructionDecoder::IsLDRSTUImm(uint32_t insn)
{
...
// Returns true if 'instr' encodes any of:
//   - RET   (0xD65F03C0)
//   - RETAA (0xD65F03E0)
//   - RETAB (0xD65F03E1)
bool CGPInstructionDecoder::IsFunctionRet(uint32_t instr)
{
    // Mask off register and hint bits (low 5 bits)
    constexpr uint32_t RET_MASK = 0xFFFFFC1Fu;

    // Canonical encodings:
    constexpr uint32_t RET_OPCODE   = 0xD65F03C0u;
    constexpr uint32_t RETAA_OPCODE = 0xD65F03E0u;  // PAC-auth with key A :contentReference[oaicite:4]{index=4}
    constexpr uint32_t RETAB_OPCODE = 0xD65F03E1u;  // PAC-auth with key B :contentReference[oaicite:5]{index=5}

    uint32_t top = instr & RET_MASK;
    return (top == RET_OPCODE
         || top == RETAA_OPCODE
         || top == RETAB_OPCODE);
}

size_t CGPInstructionDecoder::GetFunctionSize(void* funcAddr, size_t maxBytes)
{
    auto ptr = reinterpret_cast<uint8_t*>(funcAddr);

    // Ensure we only step in 4-byte increments
    size_t scanned = 0;
    while (scanned + 4 <= maxBytes)
    {
        uint32_t instr = *reinterpret_cast<uint32_t*>(ptr + scanned);
        scanned += 4;
        if (IsFunctionRet(instr))
        {
            return scanned;
        }
    }
    return -1;  // no RET found within limit
}
